<%@ Page Title="Articles" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="listings.aspx.cs" Inherits="RB2017.listings" %>
<asp:Content ID="Content1" ContentPlaceHolderID="MainContent" runat="server">
<script type="text/javascript">
    $('#articles').addClass('articles');
</script>
<%--<h3>Hypertext Markup Language (HTML)</h3>
<p><a href="articles/creating-a-html-web-page/default.aspx">Creating a HTML web page</a></p>--%>
<h3>Cascading Style Sheets (CSS)</h3>
<%--<p><a href="articles/introduction-to-css/default.aspx">Introduction to CSS</a></p>--%>
<p><a href="articles/creating-a-cascading-style-sheet-layout/default.aspx">Creating a Cascading Style Sheet Layout</a></p>
<p><a href="articles/creating-a-cascading-style-sheet-print-page/default.aspx">Creating a Cascading Style Sheet Print Page</a></p>
<p><a href="articles/creating-a-cascading-style-sheet-web-form/default.aspx">Creating a Cascading Style Sheet Web Form</a></p>
<h3>Active Server Pages.NET (MVC)</h3>
<p><a href="articles/creating-a-asp-mvc-contact-form/default.aspx">Creating a ASP MVC Contact Form</a></p>
<h3>Produce a website build</h3>
<p><a href="articles/produce-a-website-build-and-deploy/default.aspx">Building and Deploying a ASP.NET Web Application to production</a></p>
<h3>Active Server Pages.NET (ASPX)</h3>
<p><a href="articles/creating-a-asp-net-contact-form/default.aspx">Creating a ASP.NET Contact Form</a></p>
<p><a href="articles/sending-a-html-and-plain-text-e-newsletter-asp-net/default.aspx">Sending an HTML and Plain Text E-newsletter with ASP.NET</a></p>
<h3>Active Server Pages.NET (ASPX) and MS SQL Server</h3>
<p><a href="articles/creating-a-searchable-online-employee-directory-asp-net-ms-sql-server/default.aspx">Creating a Searchable Online Employee Directory with ASP.NET and MS SQL Server</a></p>
<p><a href="articles/creating-a-human-resources-application-asp-net-ms-sql-server/default.aspx">Creating a Human Resources Application with ASP.NET and MS SQL Server</a></p>
<p><a href="articles/creating-a-audit-record-application-asp-net-ms-sql-server/default.aspx">Creating an Audit Record Application with ASP.NET and MS SQL Server</a></p>
<p><a href="articles/creating-a-web-based-schedule-asp-net-ms-sql-server/default.aspx">Creating a Web-based Schedule with ASP.NET and MS SQL Server Server</a></p>
<p><a href="articles/sending-a-html-and-plain-text-e-newsletter-asp-net-part-2/default.aspx">Sending an HTML and Plain Text E-newsletter with ASP.NET, Part 2</a></p>
<p><a href="articles/creating-a-e-newsletter-signup-system-asp-net-ms-sql-server/default.aspx">Creating an E-newsletter Signup System with ASP.NET and MS SQL Server</a></p>
<p><a href="articles/creating-a-help-desk-web-application-asp-net-ms-sql-server/default.aspx">Creating a Help Desk Web Application Using ASP.NET and MS SQL Server</a></p>
<p><a href="articles/creating-a-registration-and-confirmation-system-asp-net-ms-sql-server/default.aspx">Creating a Registration and Confirmation System with ASP.NET and MS SQL Server</a></p>
<h3>Asynchronous JavaScript & XML (AJAX)</h3>
<p><a href="articles/asynchronous-javascript-and-xml/default.aspx">Explanation of long running form submit</a></p>
</asp:Content>