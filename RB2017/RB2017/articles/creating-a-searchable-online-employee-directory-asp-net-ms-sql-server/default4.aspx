...
<li>In the Add New Item window, from the left pane, choose Web, then Web Form</li>
<li>From the right pane, select Web Form</li>
<li>In the name text field, type viewdetail.aspx and left click Add.</li>
</ol>
<p>Double click viewdetail.aspx from the solution explorer and remove everything.
Next, add the following markup as shown below:</p>
<pre><code>
...
<li>In the add new item window, from the left pane, choose web, then General.</li>
...
<ol>
<li>Left click the plus sign (+) next to viewdetail.aspx.</li>
<li>Double click viewdetail.aspx.cs. Inside your partial class, add the following code as shown below:</li>
</ol>
<pre><code>
...
<ol>
<li>Create a variable with a data type of GUID</li>
<li>Use TryParse within a check on our query string in case no guid is passed.
<ul>
...
<p>You may wonder why we had to use try parse on the guid within our query string. The reason is simple: if for any reason we 
don't get a query string passed in with a guid, we need a way to handle that and show our no results found place holders. 
Otherwise, our application will throw an exception because we're accessing a null object.</p>
...
<p>Save your file. Run the project by pressing F5, and type D or Doe in the search field, which returns Doe. 
Click on the hyperlink and you should be able to view additional details regarding our employee.</p>
<h3>Summary</h3>
